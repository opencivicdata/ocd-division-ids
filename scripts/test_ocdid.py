...
test_ocdids = [
    ('ocd-division/country:us/court_of_appeals:10/district_court:colorado',
     True),
    ('ocd-division/country:us', True),
    ('ocd-division/country:us/state:ny/county:montgomery/place:some_town',
     False),
    ('invalid_ocdid', False)
]

test_match_name = [
    ('ocd-division/country:us/state:sd/county:beadle', 'place',
     'belle_prairie',
     ('ocd-division/country:us/state:sd/county:beadle/place:belle_prairie',
      100)),
    ('ocd-division/country:us/state:al', 'county', 'marion',
     ('ocd-division/country:us/state:al/county:marion', 100)),
    ('ocd-division/country:us/state:al', 'county', '3', (None, -1))
]

test_match_type = [
    ('ocd-division/country:us/state:sd/place:sioux_falls',
     'council', 5, 'council_district'),
...
class TestOcdid(unittest.TestCase):
    ...
    def testMatchName(self):
        for prefix, dist_type, dist_name, result in test_match_name:
            self.assertEqual(
                result, ocdid.match_name(prefix, dist_type, dist_name))

    def testMatchType(self):
        for prefix, dist_type, dist_len, result in test_match_type:
            self.assertEqual(
                result, ocdid.match_type(prefix, dist_type, dist_len))
    ...
    def testTypeNameSearch(self):
        ...
        for dist_type, dist_name, match_count in test_type_name_search:
            self.assertEqual(match_count,
                             len(ocdid.type_name_search(dist_type, dist_name)))
