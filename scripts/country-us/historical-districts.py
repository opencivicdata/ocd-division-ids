...
# Script that parses legislators-historical.yaml from unitedstates/congress-legislators and extracts now-defunct districts from that document. 
...
HISTORIC_LEGISLATOR_FILE = os.path.join(SCRIPT_DIRECTORY, '../../cache/legislators-historical.yaml')
...
def parse_division_id(division_id):
  ...
  if re.search("(state|territory):([a-zA-Z]{2})", division_id):
    state = re.search("(state|territory):([a-zA-Z]{2})", division_id).group(2)
  else:
    return False
  return (state, district)

def make_division_id(state, district):
  return "ocd-division/country:us/state:{state}/cd:{district}".format(state=state,district=district)

def make_division_name(state, district):
  return "{state}'s {district} congressional district".format(state=states.lookup(state).name, district=ordinalize(district))
...
def download_historic_legislators():
  ...
  f = open(os.path.join(SCRIPT_DIRECTORY, "../../cache/legislators-historical.yaml"), 'w')
  f.write(raw_historic_legislators)
  f.close()

def extract_historical_districts():
  historic_legislator_list = yaml_load(os.path.join(SCRIPT_DIRECTORY, '../../cache/legislators-historical.yaml'))
  ...
  for leg in historic_legislator_list:
    for term in leg['terms']:
      if 'district' in term:
        ...
        if (term['district'] == -1): # (value of -1 specifies unknown district)
...
def extract_existing_districts():
  congressional_district_id_file = os.path.join(SCRIPT_DIRECTORY, '../../identifiers/country-us/census_autogenerated/us_congressional_districts.csv')
...
def extract_at_large_districts():
  state_division_id_file = os.path.join(SCRIPT_DIRECTORY, '../../identifiers/country-us/states.csv')
  territory_division_id_file = os.path.join(SCRIPT_DIRECTORY, '../../identifiers/country-us/us_territories.csv')
...
def write_missing_districts(missing_districts):
  new_file_path = os.path.join(SCRIPT_DIRECTORY, '../../identifiers/country-us/historical/unitedstates_legislators_autogenerated/historical_congressional_districts.csv')
  
  print("Writing {number} missing districts".format(number=len(missing_districts)))
