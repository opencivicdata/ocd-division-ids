...
"""
BASE = 'https://raw.github.com/opencivicdata/ocd-division-ids'
OCDID_DATA = BASE + '/master/identifiers/country-us.csv'
EXCEPTION_DATA = BASE + '/master/identifiers/country-us/exceptions.csv'
MATCH_RATIO = 90
MATCH_LIMIT = 10
SEARCH_CONVERSIONS = {
    ...
    for k, v in ocdids[ocdid_prefix].iteritems():
        ...
        if new_diff_len < diff_len:
            ...
            type_ratio = new_type_ratio
                ...
                new_diff_len == diff_len and
                dist_type != 'district' and
                new_type_ratio > type_ratio
        ):
            key = k
            type_ratio = new_type_ratio
...
for line in r.text.split('\n'):
    ...
    if (
            len(line) > 2 and line[0][:line[0].rfind(':')]
            !=
            line[1][:line[0].rfind(':')]
            and not line[2].startswith('Doesn\'t exist')):
